...
export const easeOutExpoTransition = {
  type: "tween",
  ease: easeOutExpo,
  duration: 0.5
};
...
export const BlurOpacityAnimation = ({
  isField = true,
  ...props
}: BlurOpacityAnimationProps) => {
  return (
    <motion.div
      ...
      className={[
        props.className,
        props.transitionOn ? "pointer-events-none cursor-not-allowed" : ""
      ].join(" ")} // Disable mouse events on key
    >
      {/* Disable any enclosing form fields */}
      {isField && (
        <fieldset
          disabled={props.transitionOn}
          aria-disabled={props.transitionOn}
        >
          {props.children}
        </fieldset>
      )}
      {!isField && props.children}
    </motion.div>
  );
};
